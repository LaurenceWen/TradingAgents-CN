...
除发布检查报告外，你还要负责整理一份“本次版本相对上一版本的改进说明文档”，用于对外或对内说明这次版本到底比上一版提升了什么。
...
- 输出一份面向发布收尾的版本改进说明文档草稿，说明目标版本相对上一版本的核心提升点。
...
- 默认采用只读分析模式；在只读模式下，你可以先给出版本改进说明文档正文草稿，但不要落盘写文件。
- 如果用户已经明确要求你执行完整发布、生成最终物料、写入发布结果或落地收尾文档，则你应额外生成并写入一份版本改进说明文档。
...
8. 基于版本差异和已验证事实，整理一份“目标版本相对上一版本的改进说明文档”。

## 版本改进说明文档规则
- 文档目标：回答“这次版本比上一版本具体改进了什么”，避免泛泛而谈。
- 文档受众：默认以最终用户和发布负责人都能快速理解为目标，语言应偏业务可读，而不是只写技术细节。
- 文档必须基于已核实的仓库事实、release notes、git 差异和本次发布验证结果，不要编造功能点。
- 文档应优先归纳用户可感知提升，再补充必要的技术改进与兼容性提示。
- 如果已经进入实际发布/归档模式，默认文档文件名使用 `VERSION_IMPROVEMENTS_<target_version>.md`。
- 如果已经有归档目录 `D:/release/<version>/revNNN/`，优先把该文档写入归档目录；如用户明确要求写入仓库，也可同步写入仓库中的发布文档位置。
- 如果只是只读分析模式，不写文件，但在最终输出中必须包含这份文档的完整草稿或结构化正文。

推荐文档结构：
1. 标题：`<目标版本> 相对 <上一版本> 的改进说明`
2. 版本信息：目标版本、上一版本、发布日期或构建版本
3. 本次更新重点：3 到 6 条
4. 用户可感知改进：按模块归类，例如交易计划、数据同步、导出报告、安装与更新
5. 重要修复：只列用户确实会感知的问题修复
6. 升级建议：适合什么用户直接升级、升级后建议检查什么
7. 说明：如有必要，补充兼容性或发布说明入口
...
如果用户要求执行实际发版或生成最终物料，再额外生成一份 `VERSION_IMPROVEMENTS_<目标版本>.md`，内容用于说明“<目标版本> 相对 <上一版本> 有哪些改进”，并在最终输出里说明该文档的路径与摘要。

简化调用模板：

1. 准备当前版本发布
为 TradingAgentsCN 准备当前版本发布。自动识别目标版本和上一个版本，扫描 git log 与变更文件，并输出阻塞项、所需文件修改、upgrade_config 覆盖情况、migration 审查、验证清单、最终结论，以及“本次版本相对上一版本的改进说明文档”草稿。

2. 检查当前升级包是否可发
检查 TradingAgentsCN 当前版本升级包是否可以安全发布。自动识别目标版本和上一个版本，重点检查打包清单、更新器替换清单、upgrade_config 和 migration，并补充整理一份相对上一版本的改进说明摘要。
