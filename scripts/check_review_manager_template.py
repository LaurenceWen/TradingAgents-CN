"""检查 review_manager_v2 的模板内容"""
import asyncio
from motor.motor_asyncio import AsyncIOMotorClient
from dotenv import load_dotenv
import os

load_dotenv()

async def check_template():
    """检查 review_manager_v2 的模板内容"""
    # 连接数据库
    mongo_uri = os.getenv("MONGO_URI", "mongodb://localhost:27017")
    client = AsyncIOMotorClient(mongo_uri)
    db = client["tradingagents"]
    
    # 查询 review_manager_v2 的模板
    templates = await db["agent_prompts"].find({
        "agent_type": "reviewers_v2",
        "agent_name": "review_manager_v2",
        "prompt_type": "user"
    }).to_list(length=10)
    
    print(f"\n📊 review_manager_v2 的用户提示词模板：\n")
    for i, template in enumerate(templates, 1):
        template_id = template.get("_id")
        preference_type = template.get("preference_type", "N/A")
        content = template.get("content", "")
        
        print(f"{i}. template_id: {template_id}")
        print(f"   preference_type: {preference_type}")
        print(f"   内容长度: {len(content)}")
        
        # 检查是否包含评分相关的内容
        if "1-10" in content or "1–10" in content:
            print(f"   ⚠️ 发现 10 分制评分要求！")
            # 打印相关行
            lines = content.split('\n')
            for j, line in enumerate(lines):
                if "1-10" in line or "1–10" in line or "score" in line.lower():
                    print(f"      第 {j+1} 行: {line.strip()}")
        
        if "0-100" in content or "0–100" in content or "百分制" in content:
            print(f"   ✅ 已使用百分制评分！")
        
        print()
    
    client.close()

if __name__ == "__main__":
    asyncio.run(check_template())

