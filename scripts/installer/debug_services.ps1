...
"@
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-Host "    ⚠️ MongoDB连接测试: $output" -ForegroundColor Yellow
                return $false
            }
        } catch {
...
function Start-MongoDBWithLogs($mongoExe, $mongoData, $mongoPort, $showLogs) {
    ...
    try {
        ...
        if ($showLogs) {
            Write-Host "    📋 MongoDB启动日志:" -ForegroundColor Cyan
            try {
                $stdout = ""
                $stderr = ""
        ...
        if ($process.HasExited) {
            ...
            try {
                ...
                if ($errorOutput) {
                    ...
                    if ($errorOutput -match "data.*corrupt|lock.*file") {
                        Write-Host "    💡 诊断: 数据文件可能损坏，尝试删除锁文件" -ForegroundColor Cyan
                    }
                }
            } catch {
                # Ignore read errors
...
function Diagnose-MongoDB($envMap, $root, $start, $showLogs, $testConnection) {
    ...
    if ($start -and -not $procInfo.Exists) {
        ...
        if ($process) {
            ...
            if ($procInfo.Exists) {
                ...
                Write-Host "  ❌ MongoDB启动失败，进程已退出" -ForegroundColor Red
            }
        } else {
    ...
    if ($procInfo.Exists -and $portInfo.InUse -and $portInfo.ProcessName -eq "mongod") {
        ...
        if (-not $portInfo.InUse) {
            ...
            Write-Host "      - 使用 -Start 参数启动MongoDB" -ForegroundColor Gray
...
function Diagnose-Redis($envMap, $root, $start, $showLogs, $testConnection) {
    ...
    if ($procInfo.Exists -and $portInfo.InUse -and $portInfo.ProcessName -eq "redis-server") {
        ...
        if (-not $portInfo.InUse) {
            ...
            Write-Host "      - 使用 -Start 参数启动Redis" -ForegroundColor Gray
            Write-Host "      - 检查防火墙是否阻止了端口" -ForegroundColor Gray
            Write-Host ""
        }
