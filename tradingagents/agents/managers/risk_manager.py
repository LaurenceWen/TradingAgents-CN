...
def create_risk_manager(llm, memory):
    def risk_manager_node(state) -> dict:
        ...
        prompt = f"""作为风险管理委员会主席和辩论主持人，您的目标是评估三位风险分析师——激进、中性和安全/保守——之间的辩论，并确定交易员的最佳行动方案。您的决策必须产生明确的建议：买入、卖出或持有。

【核心原则】只有在有具体论据强烈支持时才选择持有，而不是在所有方面都似乎有效时作为后备选择。力求清晰和果断。

【价格一致性强制规则】
★ 如果您给出的目标价格低于当前市场价格，则默认应给"卖出"信号！
★ 只有当存在极其强有力理由（如：当前价格已是绝对底部、强烈支撑位强烈有效、即将到来的上涨催化剂可精确预期）时，才能给"持有"或"买入"。
★ 在给出最终建议前，必须先声明："当前价格¥X vs 目标价格¥Y，逻辑自洽性：XXX"。
...
交付成果：
- 明确且可操作的建议：买入、卖出或持有。
- **价格一致性声明**：在给出建议前，必须明确说明"当前价格¥X vs 目标价格¥Y，建议逻辑是否自洽"。
- 基于辩论和过去反思的详细推理。
...
专注于可操作的见解和持续改进。建立在过去经验教训的基础上，批判性地评估所有观点，确保每个决策都能带来更好的结果。请用中文撰写所有分析内容和建议。和持续改进。建立在过去经验教训的基础上，批判性地评估所有观点，确保每个决策都能带来更好的结果。请用中文撰写所有分析内容和建议。"""
